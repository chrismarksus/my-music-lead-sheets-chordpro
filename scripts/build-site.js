...
.capo-fret-pill {
  ...
  border: 1px solid var(--divider); border-radius: 999px; padding: 5px 12px;
}
#capo-fret-value { color: var(--accent-text); }
...
.song-control-bars::-webkit-scrollbar { display: none; }
.song-control-bar {
  display: flex; flex-wrap: nowrap; flex: none; align-items: center; gap: 12px; background: var(--surface);
...
.autoscroll-bar, .transpose-bar { display: flex; flex: none; align-items: center; gap: 12px; }
...
// autoscroll speed). Chords this can't parse are left exactly as rendered —
// mirrors chord-diagrams.js's "silently skip what can't be resolved"
// philosophy. Diagram tooltips are baked in at build time for the original
// key only, so they're suppressed via the body.transpose-active CSS rule
// (see CHROME_CSS) whenever offset !== 0, rather than risk showing a
// now-wrong fretboard.
...
(function () {
  ...
  var capoFretValue = document.getElementById('capo-fret-value');
  ...
  function render() {
    ...
    if (capoFretValue) capoFretValue.textContent = offset;
...
function pageShell({ title, bodyHtml, isSongPage, description, hasChords }) {
...
<span id="autoscroll-speed-label">3&times;</span>
  ...
  const transposeBarHtml = isSongPage && hasChords
    ? `<div class="transpose-bar" role="group" aria-label="Transpose controls">
...
function main() {
  ...
  files.forEach((filename) => {
    ...
    const spotifyContentHtml = spotify
      ? `<a href="${escapeHtml(spotify.url)}" target="_blank" rel="noopener"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.42 1.56-.299.421-1.02.599-1.559.3z"/></svg>Listen on Spotify</a><button type="button" class="spotify-play-btn" data-track-id="${escapeHtml(spotify.id)}">&#9654; Play preview</button>`
      : '';
    const capoFretHtml = hasChords
      ? `<span id="capo-fret-pill" class="capo-fret-pill">Capo Fret <span id="capo-fret-value">0</span></span>`
      : '';
    const spotifyLinkHtml = (spotifyContentHtml || capoFretHtml)
      ? `<p class="spotify-link">${spotifyContentHtml}${capoFretHtml}</p>${spotify ? '<div class="spotify-embed" hidden></div>' : ''}`
      : '';
    ...
    entries.push({ title, artist, genre, slug, bodyHtml, hasChords });
  });
  ...
  entries.forEach((entry, i) => {
    ...
    fs.writeFileSync(
      ...
      pageShell({ title: entry.title, bodyHtml, isSongPage: true, description, hasChords: entry.hasChords })
    );
  });
